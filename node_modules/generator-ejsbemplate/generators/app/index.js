...
module.exports = yeoman.extend({
  ...
  writing: function () {
    ...
    this.fs.copyTpl(
      ...
      { name:baseName, fName:fName, camelName:baseName.replace(/[-_]([a-z])/g, (g) => { return g[1].toUpperCase(); }), end:'%>' }
    );

    this.fs.copyTpl(
      this.templatePath('_template-preview.ejs'),
      this.destinationPath(baseName + '/_' + baseName + '-preview.ejs'),
      {name: baseName, fName:fName, end: '%>'}
    );

    this.fs.copyTpl(
      this.templatePath('_template-data.ejs'),
      this.destinationPath(baseName + '/_' + baseName + '-data.ejs'),
      {name: baseName, fName:fName, end: '%>'}
    );
